...
test('current repository findings and exact task bindings are source-backed', () => {
  ...
  assert.deepEqual(byID.get('DIFF_FACTORY_NOT_WIRED').taskIDs, [
    'P05-T112', 'P07-T009', 'P07-T010',
  ]);
  assert.deepEqual(byID.get('SAMPLE_HOST_DIFF_ACTIVATION_ABSENT').taskIDs, [
    'P07-T009', 'P07-T010',
  ]);
  assert.deepEqual(byID.get('MARKER_SERVICE_ABSENT').taskIDs, [
    'P05-T001', 'P05-T012', 'P05-T122',
  ]);
  ...
  assert.deepEqual(byID.get('WIDGET_MOUSE_TARGET_SURFACE_EMPTY').taskIDs, [
    'P03-T007', 'P05-T012', 'P05-T104', 'P05-T116', 'P05-T117',
    'P05-T118', 'P05-T125', 'P05-T146', 'P05-T151', 'P05-T153',
  ]);
  assert.deepEqual(byID.get('LANGUAGE_CONTEXT_TYPES_EMPTY').taskIDs, [
    'P05-T001', 'P05-T013', 'P06-T005',
  ]);
  assert.deepEqual(
    byID.get('FEATURE_ACTIVATION_PATH_ABSENT').taskIDs,
    Array.from({ length: 62 }, (_, index) => `P05-T${String(index + 100).padStart(3, '0')}`),
  );
