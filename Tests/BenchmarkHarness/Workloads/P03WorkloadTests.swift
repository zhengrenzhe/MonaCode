// P03WorkloadTests.swift
//
// P09-T033 — Run P03: batch edits (the fourth P-candidate).
//
// P03 is the batch-edits performance benchmark: preparing and committing
// 1, 100, and 10000 non-overlapping edits while measuring the preparation
// and commit components separately, then the 1,000,000-draw whole-balanced-
// block bootstrap verdict across the exact 60 Hz and 120 Hz frame cells for
// the M0 and M1 comparators, with the one-sided 95 percent upper bound
// required to be at most zero on unrounded binary64 values for every
// comparator and cell.
//
// ──────────────────────────────────────────────────────────────────────────
// FEASIBILITY ASSESSMENT (P09-T033) — Option A (structural)
// ──────────────────────────────────────────────────────────────────────────
...
//    reduced batch-edit measurement requires the npm package + a Node.js
...
// external oracle harness); this test verifies the P03 configuration is
// correctly wired against the Phase 00 statistical + display + manifest
// infrastructure.
//
// Spec (verbatim, P09-T033 implementation-operation stages):
//   • Prepare and commit 1, 100, and 10000 non-overlapping edits while
//     measuring preparation and commit components separately.
...
// MARK: - P03 workload configuration (documented, verified against infra)

/// The P03 batch-edits workload configuration. These are the measurement-time
...
/// run.
private enum P03Configuration {
    /// The three batch sizes the formal run measures — 1, 100, and 10000
    /// non-overlapping edits per block. Each size is a distinct cell axis.
    static let batchSizes: [Int] = [1, 100, 10_000]
    /// The two components measured separately — preparation (building the
    /// edit batch) and commit (applying it). Each is a distinct cell with
    /// its own verdict; they are never merged.
    static let components: [String] = ["preparation", "commit"]
    /// 30 hot balanced blocks per valid cell. P03 is a HOT workload — the
    ...
    /// The cold path's per-block launch count (P00 only). P03 is hot, so its
    ...
    /// Separate total and component metrics — preparation latency and
    /// commit latency are distinct cells with their own verdicts, plus the
    /// total batch latency; they are never merged.
...
// MARK: - P03WorkloadTests

final class P03WorkloadTests: XCTestCase {

    // MARK: - Stage 1: 1, 100, 10000 non-overlapping edits; prep + commit

    /// P03 measures three batch sizes: 1, 100, and 10000 non-overlapping
    /// edits. (spec: "Prepare and commit 1, 100, and 10000 non-overlapping
    /// edits …")
    func testP03BatchSizesAreOneHundredAndTenThousand() {
        XCTAssertEqual(P03Configuration.batchSizes, [1, 100, 10_000])
        XCTAssertEqual(P03Configuration.batchSizes.count, 3)
        // Each is a power-of-ten step (1, 10^2, 10^4).
        XCTAssertEqual(P03Configuration.batchSizes.first, 1)
        XCTAssertEqual(P03Configuration.batchSizes.last, 10_000)
    }

    /// The two components measured separately — preparation and commit.
    /// (spec: "…while measuring preparation and commit components
    /// separately.")
    func testP03ComponentsArePreparationAndCommit() {
        XCTAssertEqual(P03Configuration.components, ["preparation", "commit"])
        XCTAssertEqual(P03Configuration.components.count, 2)
    }
    ...
    func testHotBlockCountIsThirty() throws {
        let manifest = makeManifest(minimumSamples: P03Configuration.hotBlocksPerCell)
        let stats = BootstrapStatistics()
        let blocks = makeBlocks(count: P03Configuration.hotBlocksPerCell)
    ...
    /// P03 is a HOT workload — its block count (30) is governed by the
    ...
    func testHotWorkloadDistinctFromColdLaunchPath() {
        XCTAssertEqual(
            P03Configuration.hotBlocksPerCell, 30,
            "P03 uses 30 HOT blocks per cell."
        )
        XCTAssertEqual(
            ColdLaunchManager.samplesPerBlock,
            P03Configuration.coldPathLaunchesPerBlock,
            "The cold path (P00) uses 50 launches per block; P03 is hot."
        )
        XCTAssertNotEqual(
            P03Configuration.hotBlocksPerCell,
            P03Configuration.coldPathLaunchesPerBlock,
    ...
    func testNoOutlierDeletionAllBlocksEnterBootstrap() throws {
        ...
        XCTAssertEqual(verdict.bootstrapDrawCount, P03Configuration.bootstrapDrawCount)
    }

    /// Preparation and commit are DISTINCT component cells — each carries its
    /// own cellID and produces its own verdict; they are never merged. The
    /// manifest's `cellID` encodes the component, so the two cells seed the
    /// bootstrap independently. (spec: "measuring preparation and commit
    /// components separately".)
    func testPreparationAndCommitAreDistinctComponentCells() {
        let prepManifest = makeManifest(
            cellID: "p03-batch10000-60hz-preparation-native-vs-m0"
        )
        let commitManifest = makeManifest(
            cellID: "p03-batch10000-60hz-commit-native-vs-m0"
        )
        XCTAssertNotEqual(
            prepManifest.cellID, commitManifest.cellID,
            "Preparation and commit are distinct component cells."
        )
        let prepMaterial = BootstrapStatistics.seedMaterial(for: prepManifest)
        let commitMaterial = BootstrapStatistics.seedMaterial(for: commitManifest)
        XCTAssertNotEqual(
            prepMaterial, commitMaterial,
            "Preparation and commit cells must seed the bootstrap independently."
        )
    }
    ...
    func testBootstrapDrawCountIsExactlyOneMillion() throws {
        ...
        XCTAssertEqual(
            ...
            P03Configuration.bootstrapDrawCount
        )
    ...
    /// — for P03 that means both the M0 cell and the M1 cell must pass. No
    ...
    func testRefreshCellsAreExact60And120() {
        XCTAssertEqual(
            ...
            P03Configuration.exactRefreshRates
        )
    ...
    func testBootstrapSeedMaterialIsReproducibleAndCellScoped() {
        let m0 = makeManifest(cellID: "p03-batch10000-60hz-total-native-vs-m0")
        let m1 = makeManifest(cellID: "p03-batch10000-60hz-total-native-vs-m1")
    ...
    // MARK: - Full P03 pipeline wiring (synthetic data — NOT a performance
    //          result)

    /// End-to-end wiring proof: the P03 pipeline — collect balanced blocks
    /// → run each verdict form → aggregate via intersection-union — is wired
    /// correctly. Uses SYNTHETIC blocks (not real batch-edit timings); this
    ...
    /// Both the preparation and commit component cells are wired.
    func testP03FullPipelineWiringWithSyntheticBlocks() throws {
        let runner = BenchmarkRunner()
        let stats = BootstrapStatistics()
        ...
        // The preparation component cell (M0 + M1, each its own verdict).
        let prepM0 = makeManifest(
            cellID: "p03-batch10000-60hz-preparation-native-vs-m0",
            comparator: .m0, minimumSamples: 30
        )
        let prepM1 = makeManifest(
            cellID: "p03-batch10000-60hz-preparation-native-vs-m1",
            comparator: .m1, minimumSamples: 30
        )
        let prepM0Verdict = try stats.positiveRatioVerdict(blocks: blocks, manifest: prepM0)
        let prepM1Verdict = try stats.positiveRatioVerdict(blocks: blocks, manifest: prepM1)

        // The commit component cell (M0 + M1, each its own verdict).
        let commitM0 = makeManifest(
            cellID: "p03-batch10000-60hz-commit-native-vs-m0",
            comparator: .m0, minimumSamples: 30
        )
        let commitM1 = makeManifest(
            cellID: "p03-batch10000-60hz-commit-native-vs-m1",
            comparator: .m1, minimumSamples: 30
        )
        let commitM0Verdict = try stats.positiveRatioVerdict(blocks: blocks, manifest: commitM0)
        let commitM1Verdict = try stats.positiveRatioVerdict(blocks: blocks, manifest: commitM1)

        // Every comparator cell's upper bound is at most zero on unrounded
        // binary64 (synthetic native < comparator → passes).
        XCTAssertTrue(prepM0Verdict.passes)
        XCTAssertTrue(prepM1Verdict.passes)
        XCTAssertTrue(commitM0Verdict.passes)
        XCTAssertTrue(commitM1Verdict.passes)
        XCTAssertEqual(prepM0Verdict.bootstrapDrawCount, 1_000_000)
        XCTAssertEqual(commitM0Verdict.bootstrapDrawCount, 1_000_000)

        // Intersection-union: global no-regression requires EVERY cell
        // (both components × both comparators) to pass.
        XCTAssertTrue(stats.intersectionUnion(verdicts: [
            prepM0Verdict, prepM1Verdict, commitM0Verdict, commitM1Verdict
        ]))
    }
    ...
    private func makeManifest(
        cellID: String = "p03-batch10000-60hz-total-native-vs-m0",
