// P07WorkloadTests.swift
//
// P09-T037 — Run P07: decorations (the eighth P-candidate).
//
// P07 is the decorations performance benchmark: updating 100000 model
// decorations with 10000 visible/offscreen mixed actions and exact
// interval-operation counters, measured as per-action latency, then the
...
//
// ──────────────────────────────────────────────────────────────────────────
// FEASIBILITY ASSESSMENT (P09-T037) — Option A (structural)
// ──────────────────────────────────────────────────────────────────────────
...
//    reduced decorations measurement requires the npm package + a Node.js
...
// external oracle harness); this test verifies the P07 configuration is
// correctly wired against the Phase 00 statistical + display + manifest
// infrastructure.
//
// Spec (verbatim, P09-T037 implementation-operation stages):
//   • Update 100000 model decorations with 10000 visible/offscreen mixed
//     actions and exact interval-operation counters.
...
// MARK: - P07 workload configuration (documented, verified against infra)

/// The P07 decorations workload configuration. These are the measurement-time
...
/// the formal run.
private enum P07Configuration {
    /// 100000 model decorations — the decoration-tree size the formal run
    /// updates. (spec: "Update 100000 model decorations …")
    static let modelDecorations: Int = 100_000
    /// 10000 actions per measurement block — each block performs 10000
    /// decoration updates (insert, query, delete) drawn from the visible/
    /// offscreen mixed action set.
    /// (spec: "…with 10000 visible/offscreen mixed actions …")
    static let actionsPerBlock: Int = 10_000
    /// The two action kinds the formal run mixes — visible (decorations in
    /// the current viewport) and offscreen (decorations outside it). The
    /// mix exercises both the render-relevant and render-irrelevant paths.
    /// (spec: "…visible/offscreen mixed actions …")
    static let actionKinds: [String] = ["visible", "offscreen"]
    /// Exact interval-operation counters — the formal run records the exact
    /// count of interval operations (interval-tree inserts/queries/deletes)
    /// per block, not an estimate. This is the "exact" in the spec.
    /// (spec: "…and exact interval-operation counters.")
    static let exactIntervalOperationCounters: Bool = true
    /// 30 hot balanced blocks per valid cell. P07 is a HOT workload — the
    ...
    /// The cold path's per-block launch count (P00 only). P07 is hot, so its
...
// MARK: - P07WorkloadTests

final class P07WorkloadTests: XCTestCase {

    // MARK: - Stage 1: 100000 decorations; 10000 visible/offscreen mixed
    //          actions; exact interval-operation counters

    /// P07 updates 100000 model decorations. (spec: "Update 100000 model
    /// decorations …")
    func testP07UpdatesOneHundredThousandModelDecorations() {
        XCTAssertEqual(
            P07Configuration.modelDecorations, 100_000,
            "P07 updates 100000 model decorations."
        )
    }

    /// The formal run performs 10000 decoration actions per measurement
    /// block, drawn from the visible/offscreen mixed action set.
    /// (spec: "…with 10000 visible/offscreen mixed actions …")
    func testP07ActionsPerBlockAreTenThousand() {
        XCTAssertEqual(
            P07Configuration.actionsPerBlock, 10_000,
            "P07 performs 10000 actions per block."
        )
    }

    /// The two action kinds — visible and offscreen — are mixed so both the
    /// render-relevant and render-irrelevant paths are exercised.
    /// (spec: "…visible/offscreen mixed actions …")
    func testP07ActionKindsAreVisibleAndOffscreenMixed() {
        XCTAssertEqual(P07Configuration.actionKinds.count, 2)
        XCTAssertEqual(P07Configuration.actionKinds, ["visible", "offscreen"])
    }

    /// The formal run records exact interval-operation counters — the exact
    /// count of interval-tree inserts/queries/deletes per block, not an
    /// estimate. (spec: "…and exact interval-operation counters.")
    func testP07UsesExactIntervalOperationCounters() {
        XCTAssertTrue(
            P07Configuration.exactIntervalOperationCounters,
            "P07 records exact interval-operation counters."
        )
    }
    ...
    func testHotBlockCountIsThirty() throws {
        let manifest = makeManifest(minimumSamples: P07Configuration.hotBlocksPerCell)
        let stats = BootstrapStatistics()
        let blocks = makeBlocks(count: P07Configuration.hotBlocksPerCell)
    ...
    /// P07 is a HOT workload — its block count (30) is governed by the
    ...
    /// apply to P07's hot blocks: a hot block is a paired N/C latency, not a
    /// fresh-process launch.
    func testHotWorkloadDistinctFromColdLaunchPath() {
        XCTAssertEqual(
            P07Configuration.hotBlocksPerCell, 30,
            "P07 uses 30 HOT blocks per cell."
        )
        XCTAssertEqual(
            ColdLaunchManager.samplesPerBlock,
            P07Configuration.coldPathLaunchesPerBlock,
            "The cold path (P00) uses 50 launches per block; P07 is hot."
        )
        XCTAssertNotEqual(
            P07Configuration.hotBlocksPerCell,
            P07Configuration.coldPathLaunchesPerBlock,
    ...
    func testNoOutlierDeletionAllBlocksEnterBootstrap() throws {
        ...
        XCTAssertEqual(verdict.bootstrapDrawCount, P07Configuration.bootstrapDrawCount)
    }
    ...
    func testBootstrapDrawCountIsExactlyOneMillion() throws {
        ...
        XCTAssertEqual(
            ...
            P07Configuration.bootstrapDrawCount
        )
    ...
    /// — for P07 that means both the M0 cell and the M1 cell must pass. No
    ...
    func testRefreshCellsAreExact60And120() {
        XCTAssertEqual(
            ...
            P07Configuration.exactRefreshRates
        )
    ...
    func testBootstrapSeedMaterialIsReproducibleAndCellScoped() {
        let m0 = makeManifest(cellID: "p07-decorations-60hz-total-native-vs-m0")
        let m1 = makeManifest(cellID: "p07-decorations-60hz-total-native-vs-m1")
    ...
    // MARK: - Full P07 pipeline wiring (synthetic data — NOT a performance
    //          result)

    /// End-to-end wiring proof: the P07 pipeline — collect balanced blocks
    /// → run each verdict form → aggregate via intersection-union — is wired
    /// correctly. Uses SYNTHETIC blocks (not real decoration-update timings);
    ...
    func testP07FullPipelineWiringWithSyntheticBlocks() throws {
        let runner = BenchmarkRunner()
        let stats = BootstrapStatistics()
        ...
        let m0Manifest = makeManifest(
            cellID: "p07-decorations-60hz-total-native-vs-m0",
            comparator: .m0, minimumSamples: 30
        )
        let m1Manifest = makeManifest(
            cellID: "p07-decorations-60hz-total-native-vs-m1",
    ...
    private func makeManifest(
        cellID: String = "p07-decorations-60hz-total-native-vs-m0",
