// P02WorkloadTests.swift
//
// P09-T032 — Run P02: typing and undo (the third P-candidate).
//
// P02 is the typing-and-undo performance benchmark: executing 10000 ABC
// typing, delete, selection, undo, and redo actions on the 1 MiU16 corpus,
// measured as per-action latency, then the 1,000,000-draw whole-balanced-
...
// comparator and cell. Multi-cursor edits are synchronized via a barrier.
//
// ──────────────────────────────────────────────────────────────────────────
// FEASIBILITY ASSESSMENT (P09-T032) — Option A (structural)
// ──────────────────────────────────────────────────────────────────────────
...
//    reduced typing/undo measurement requires the npm package + a Node.js
...
// external oracle harness); this test verifies the P02 configuration is
// correctly wired against the Phase 00 statistical + display + manifest
// infrastructure.
//
// Spec (verbatim, P09-T032 implementation-operation stages):
//   • Execute 10000 ABC typing, delete, selection, undo, and redo actions
//     on the 1 MiU16 corpus.
...
// MARK: - P02 workload configuration (documented, verified against infra)

/// The P02 typing-and-undo workload configuration. These are the measurement-
...
/// the formal run.
private enum P02Configuration {
    /// 10000 actions per measurement block (ABC typing, delete, selection,
    /// undo, and redo).
    static let actionsPerBlock: Int = 10_000
    /// The five action classes the formal run exercises.
    static let actionClasses: [String] = [
        "typing", "delete", "selection", "undo", "redo"
    ]
    /// 1 MiU16 = 2^20 = 1,048,576 UTF-16 code units (the corpus P02 edits).
    static let corpusCodeUnits: Int = 1_048_576
    /// 30 hot balanced blocks per valid cell. P02 is a HOT workload — the
    ...
    /// The cold path's per-block launch count (P00 only). P02 is hot, so its
    ...
    /// Multi-cursor edits are synchronized via a barrier — the formal run's
    /// multi-cursor contract. The barrier ensures all cursors complete a
    /// step before the next begins, so the per-action latency is a clean
    /// measurement rather than a race.
    static let multiCursorSynchronization: String = "barrier"
    ...
    /// Separate total and component metrics — typing latency and undo/redo
    /// latency are distinct cells with their own verdicts; they are never
    /// merged.
...
// MARK: - P02WorkloadTests

final class P02WorkloadTests: XCTestCase {

    // MARK: - Stage 1: 10000 actions across 5 classes on 1 MiU16 corpus

    /// P02 executes 10000 actions per block across the five action classes.
    /// (spec: "Execute 10000 ABC typing, delete, selection, undo, and redo
    /// actions on the 1 MiU16 corpus.")
    func testP02ActionsAreTenThousandAcrossFiveClasses() {
        XCTAssertEqual(P02Configuration.actionsPerBlock, 10_000)
        XCTAssertEqual(P02Configuration.actionClasses.count, 5)
        XCTAssertEqual(P02Configuration.actionClasses, [
            "typing", "delete", "selection", "undo", "redo"
        ])
    }

    /// The P02 corpus is 1 MiU16 (2^20 = 1,048,576 UTF-16 code units).
    /// (spec: "…on the 1 MiU16 corpus.")
    func testP02CorpusIsOneMebiU16() {
        XCTAssertEqual(P02Configuration.corpusCodeUnits, 1_048_576,
                       "1 MiU16 = 2^20 = 1,048,576 UTF-16 code units.")
    }

    /// Multi-cursor edits are synchronized via a barrier — the formal run's
    /// multi-cursor contract. (Structural documentation of the
    /// synchronization primitive the formal run uses.)
    func testP02MultiCursorSynchronizedViaBarrier() {
        XCTAssertEqual(
            P02Configuration.multiCursorSynchronization, "barrier",
            "P02 multi-cursor edits are synchronized via a barrier."
        )
    }
    ...
    func testHotBlockCountIsThirty() throws {
        let manifest = makeManifest(minimumSamples: P02Configuration.hotBlocksPerCell)
        let stats = BootstrapStatistics()
        let blocks = makeBlocks(count: P02Configuration.hotBlocksPerCell)
    ...
    /// P02 is a HOT workload — its block count (30) is governed by the
    ...
    /// apply to P02's hot blocks: a hot block is a paired N/C latency, not a
    /// fresh-process launch.
    func testHotWorkloadDistinctFromColdLaunchPath() {
        XCTAssertEqual(
            P02Configuration.hotBlocksPerCell, 30,
            "P02 uses 30 HOT blocks per cell."
        )
        XCTAssertEqual(
            ColdLaunchManager.samplesPerBlock,
            P02Configuration.coldPathLaunchesPerBlock,
            "The cold path (P00) uses 50 launches per block; P02 is hot."
        )
        XCTAssertNotEqual(
            P02Configuration.hotBlocksPerCell,
            P02Configuration.coldPathLaunchesPerBlock,
    ...
    func testNoOutlierDeletionAllBlocksEnterBootstrap() throws {
        ...
        XCTAssertEqual(verdict.bootstrapDrawCount, P02Configuration.bootstrapDrawCount)
    }
    ...
    func testBootstrapDrawCountIsExactlyOneMillion() throws {
        ...
        XCTAssertEqual(
            ...
            P02Configuration.bootstrapDrawCount
        )
    ...
    /// — for P02 that means both the M0 cell and the M1 cell must pass. No
    ...
    func testRefreshCellsAreExact60And120() {
        XCTAssertEqual(
            ...
            P02Configuration.exactRefreshRates
        )
    ...
    func testBootstrapSeedMaterialIsReproducibleAndCellScoped() {
        let m0 = makeManifest(cellID: "p02-typing-60hz-total-native-vs-m0")
        let m1 = makeManifest(cellID: "p02-typing-60hz-total-native-vs-m1")
    ...
    // MARK: - Full P02 pipeline wiring (synthetic data — NOT a performance
    //          result)

    /// End-to-end wiring proof: the P02 pipeline — collect balanced blocks
    /// → run each verdict form → aggregate via intersection-union — is wired
    /// correctly. Uses SYNTHETIC blocks (not real typing/undo timings); this
    ...
    func testP02FullPipelineWiringWithSyntheticBlocks() throws {
        let runner = BenchmarkRunner()
        let stats = BootstrapStatistics()
        ...
        let m0Manifest = makeManifest(
            cellID: "p02-typing-60hz-total-native-vs-m0",
            comparator: .m0, minimumSamples: 30
        )
        let m1Manifest = makeManifest(
            cellID: "p02-typing-60hz-total-native-vs-m1",
    ...
    private func makeManifest(
        cellID: String = "p02-typing-60hz-total-native-vs-m0",
