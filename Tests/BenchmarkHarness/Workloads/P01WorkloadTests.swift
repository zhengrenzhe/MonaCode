// P01WorkloadTests.swift
//
// P09-T031 — Run P01: model load (the second P-candidate).
//
// P01 is the model-load performance benchmark: loading the 1 MiU16 and
// 100 MiU16 raw-text corpora across LF, CRLF, valid Unicode, isolated-
// surrogate, and mixed-line families, measured as load latency, then the
...
//
// ──────────────────────────────────────────────────────────────────────────
// FEASIBILITY ASSESSMENT (P09-T031) — Option A (structural)
// ──────────────────────────────────────────────────────────────────────────
...
//    reduced model-load measurement requires the npm package + a Node.js
//    runtime + a real macOS app — infeasible in-session. No reduced
...
// external oracle harness); this test verifies the P01 configuration is
// correctly wired against the Phase 00 statistical + display + manifest
// infrastructure.
//
// Spec (verbatim, P09-T031 implementation-operation stages):
//   • Load 1 MiU16 and 100 MiU16 raw-text corpora across LF, CRLF, valid
//     Unicode, isolated-surrogate, and mixed-line families.
...
// MARK: - P01 workload configuration (documented, verified against infra)

/// The P01 model-load workload configuration. These are the measurement-time
...
/// run.
private enum P01Configuration {
    /// 1 MiU16 = 2^20 = 1,048,576 UTF-16 code units (the small corpus).
    static let smallCorpusCodeUnits: Int = 1_048_576
    /// 100 MiU16 = 100 * 2^20 = 104,857,600 UTF-16 code units (the large
    /// corpus). 100 MiU16 is exactly 100x the small corpus.
    static let largeCorpusCodeUnits: Int = 104_857_600
    /// The five raw-text line families the formal run loads: LF (Unix),
    /// CRLF (Windows), valid Unicode (BMP + supplementary), isolated-
    /// surrogate (unpaired high/low surrogates), and mixed (all of the
    /// above interleaved).
    static let lineFamilies: [String] = [
        "LF", "CRLF", "unicode", "isolated-surrogate", "mixed"
    ]
    /// 30 hot balanced blocks per valid cell. P01 is a HOT workload — the
    /// model is already loaded and warm; each block is a paired N/C latency
    /// measurement, NOT a fresh-process cold launch.
    static let hotBlocksPerCell: Int = 30
    /// The cold path's per-block launch count (P00 only). P01 is hot, so its
    ...
    /// being declared invalid (the formal run enforces this; the recorded
    /// cell keeps only valid blocks).
    ...
    /// its own verdict; they are never merged. For P01 the components are
    /// the load's sub-phases (e.g. parse vs. tokenize), kept separate from
    /// the total load latency.
...
// MARK: - P01WorkloadTests

final class P01WorkloadTests: XCTestCase {

    // MARK: - Stage 1: 1 MiU16 + 100 MiU16 corpora across 5 line families

    /// The P01 small corpus is 1 MiU16 (2^20 = 1,048,576 UTF-16 code units)
    /// — the same corpus size as P00. The large corpus is 100 MiU16.
    /// (spec: "Load 1 MiU16 and 100 MiU16 raw-text corpora …")
    func testP01CorporaAreOneAndOneHundredMebiU16() {
        XCTAssertEqual(
            P01Configuration.smallCorpusCodeUnits, 1_048_576,
            "1 MiU16 = 2^20 = 1,048,576 UTF-16 code units."
        )
        XCTAssertEqual(
            P01Configuration.largeCorpusCodeUnits, 104_857_600,
            "100 MiU16 = 100 * 2^20 = 104,857,600 UTF-16 code units."
        )
        XCTAssertEqual(
            P01Configuration.largeCorpusCodeUnits,
            100 * P01Configuration.smallCorpusCodeUnits,
            "100 MiU16 is exactly 100x the 1 MiU16 small corpus."
        )
    }

    /// The five line families the formal run loads. (spec: "across LF, CRLF,
    /// valid Unicode, isolated-surrogate, and mixed-line families.")
    func testP01LineFamiliesAreLF_CRLF_Unicode_Surrogate_Mixed() {
        XCTAssertEqual(P01Configuration.lineFamilies.count, 5)
        XCTAssertEqual(P01Configuration.lineFamilies, [
            "LF", "CRLF", "unicode", "isolated-surrogate", "mixed"
        ])
    }
    ...
    func testHotBlockCountIsThirty() throws {
        let manifest = makeManifest(minimumSamples: P01Configuration.hotBlocksPerCell)
        let stats = BootstrapStatistics()
        let blocks = makeBlocks(count: P01Configuration.hotBlocksPerCell)
    ...
    /// P01 is a HOT workload — its block count (30) is governed by the
    ...
    /// apply to P01's hot blocks: a hot block is a paired N/C latency, not a
    /// fresh-process launch.
    func testHotWorkloadDistinctFromColdLaunchPath() {
        XCTAssertEqual(
            P01Configuration.hotBlocksPerCell, 30,
            "P01 uses 30 HOT blocks per cell."
        )
        XCTAssertEqual(
            ColdLaunchManager.samplesPerBlock,
            P01Configuration.coldPathLaunchesPerBlock,
            "The cold path (P00) uses 50 launches per block; P01 is hot."
        )
        XCTAssertNotEqual(
            P01Configuration.hotBlocksPerCell,
            P01Configuration.coldPathLaunchesPerBlock,
    ...
    func testNoOutlierDeletionAllBlocksEnterBootstrap() throws {
        ...
        XCTAssertEqual(verdict.bootstrapDrawCount, P01Configuration.bootstrapDrawCount)
    }
    ...
    func testBootstrapDrawCountIsExactlyOneMillion() throws {
        ...
        XCTAssertEqual(
            ...
            P01Configuration.bootstrapDrawCount
        )
    ...
    /// — for P01 that means both the M0 cell and the M1 cell must pass. No
    ...
    func testRefreshCellsAreExact60And120() {
        XCTAssertEqual(
            ...
            P01Configuration.exactRefreshRates
        )
    ...
    func testBootstrapSeedMaterialIsReproducibleAndCellScoped() {
        let m0 = makeManifest(cellID: "p01-load-60hz-total-native-vs-m0")
        let m1 = makeManifest(cellID: "p01-load-60hz-total-native-vs-m1")
    ...
    // MARK: - Full P01 pipeline wiring (synthetic data — NOT a performance
    //          result)

    /// End-to-end wiring proof: the P01 pipeline — collect balanced blocks
    /// → run each verdict form → aggregate via intersection-union — is wired
    /// correctly. Uses SYNTHETIC blocks (not real model-load timings); this
    ...
    func testP01FullPipelineWiringWithSyntheticBlocks() throws {
        let runner = BenchmarkRunner()
        let stats = BootstrapStatistics()
        ...
        let m0Manifest = makeManifest(
            cellID: "p01-load-60hz-total-native-vs-m0",
            comparator: .m0, minimumSamples: 30
        )
        let m1Manifest = makeManifest(
            cellID: "p01-load-60hz-total-native-vs-m1",
    ...
    private func makeManifest(
        cellID: String = "p01-load-60hz-total-native-vs-m0",
