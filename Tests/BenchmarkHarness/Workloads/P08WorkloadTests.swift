// P08WorkloadTests.swift
//
// P09-T038 — Run P08: find and replace (the ninth P-candidate).
//
// P08 is the find-and-replace performance benchmark: running literal,
// /gimu, zero-length, and capture-replacement search/replace across density
// 0/0.1/10/1000 per KiB and RegExp-consumer, case, normalization, binary64,
// decoder, and hash matrices on 10 MiU16, measured as per-scan latency, then
// the 1,000,000-draw whole-balanced-block bootstrap verdict across the exact
// 60 Hz and 120 Hz frame cells for the M0 and M1 comparators, with the one-
// sided 95 percent upper bound required to be at most zero on unrounded
// binary64 values for every comparator and cell.
//
// ──────────────────────────────────────────────────────────────────────────
// FEASIBILITY ASSESSMENT (P09-T038) — Option A (structural)
// ──────────────────────────────────────────────────────────────────────────
...
//    reduced find/replace measurement requires the npm package + a Node.js
...
// external oracle harness); this test verifies the P08 configuration is
// correctly wired against the Phase 00 statistical + display + manifest
// infrastructure.
//
// Spec (verbatim, P09-T038 implementation-operation stages):
//   • Run literal, /gimu, zero-length, capture replacement, density
//     0/0.1/10/1000 per KiB, RegExp consumer, case, normalization, binary64,
//     decoder, and hash matrices on 10 MiU16.
...
// MARK: - P08 workload configuration (documented, verified against infra)

/// The P08 find-and-replace workload configuration. These are the
...
private enum P08Configuration {
    /// The four search/replace modes the formal run exercises — literal
    /// (plain-text find), /gimu (global, case-insensitive, multiline,
    /// unicode RegExp), zero-length (empty-match RegExp), and capture
    /// replacement (backreference substitution).
    /// (spec: "Run literal, /gimu, zero-length, capture replacement …")
    static let searchModes: [String] = [
        "literal", "/gimu", "zero-length", "capture-replacement"
    ]
    /// The four match densities (matches per KiB) the formal run sweeps — 0
    /// (no matches), 0.1 (sparse), 10 (moderate), and 1000 (dense). Each
    /// density is a distinct cell axis.
    /// (spec: "…density 0/0.1/10/1000 per KiB …")
    static let matchDensitiesPerKiB: [Double] = [0.0, 0.1, 10.0, 1000.0]
    /// The six matrices the formal run measures — RegExp consumer (the
    /// engine's RegExp NFA/bytecode consumer), case (case-folding cost),
    /// normalization (Unicode normalization cost), binary64 (binary64
    /// conversion cost), decoder (UTF-16 decoder cost), and hash (hash
    /// computation for match dedup). Each is a distinct cell with its own
    /// verdict.
    /// (spec: "…RegExp consumer, case, normalization, binary64, decoder,
    /// and hash matrices …")
    static let matrices: [String] = [
        "regexp-consumer", "case", "normalization", "binary64", "decoder", "hash"
    ]
    /// 10 MiU16 = 10 * 2^20 = 10,485,760 UTF-16 code units (the corpus P08
    /// searches and replaces in).
    /// (spec: "…on 10 MiU16.")
    static let corpusCodeUnits: Int = 10_485_760
    /// 30 hot balanced blocks per valid cell. P08 is a HOT workload — the
    ...
    /// The cold path's per-block launch count (P00 only). P08 is hot, so its
...
// MARK: - P08WorkloadTests

final class P08WorkloadTests: XCTestCase {

    // MARK: - Stage 1: literal/gimu/zero-length/capture; density 0/0.1/10/
    //          1000 per KiB; 6 matrices; 10 MiU16

    /// The four search/replace modes — literal, /gimu, zero-length, and
    /// capture-replacement. (spec: "Run literal, /gimu, zero-length, capture
    /// replacement …")
    func testP08SearchModesAreLiteralGimuZeroLengthCaptureReplacement() {
        XCTAssertEqual(P08Configuration.searchModes.count, 4)
        XCTAssertEqual(P08Configuration.searchModes, [
            "literal", "/gimu", "zero-length", "capture-replacement"
        ])
    }

    /// The four match densities (matches per KiB) — 0, 0.1, 10, and 1000.
    /// Each density is a distinct cell axis. (spec: "…density 0/0.1/10/1000
    /// per KiB …")
    func testP08MatchDensitiesAreZero_PointOne_Ten_ThousandPerKiB() {
        XCTAssertEqual(P08Configuration.matchDensitiesPerKiB.count, 4)
        XCTAssertEqual(P08Configuration.matchDensitiesPerKiB, [0.0, 0.1, 10.0, 1000.0])
        // 0 density = no matches (worst case for literal search overhead).
        XCTAssertEqual(P08Configuration.matchDensitiesPerKiB.first, 0.0)
        // 1000 per KiB = dense matches (worst case for replacement volume).
        XCTAssertEqual(P08Configuration.matchDensitiesPerKiB.last, 1000.0)
    }

    /// The six matrices — RegExp consumer, case, normalization, binary64,
    /// decoder, and hash. Each is a distinct cell with its own verdict.
    /// (spec: "…RegExp consumer, case, normalization, binary64, decoder, and
    /// hash matrices …")
    func testP08MatricesAreRegexpConsumerCaseNormalizationBinary64DecoderHash() {
        XCTAssertEqual(P08Configuration.matrices.count, 6)
        XCTAssertEqual(P08Configuration.matrices, [
            "regexp-consumer", "case", "normalization", "binary64", "decoder", "hash"
        ])
    }

    /// The P08 corpus is 10 MiU16 (10 * 2^20 = 10,485,760 UTF-16 code units).
    /// (spec: "…on 10 MiU16.")
    func testP08CorpusIsTenMebiU16() {
        XCTAssertEqual(
            P08Configuration.corpusCodeUnits, 10_485_760,
            "10 MiU16 = 10 * 2^20 = 10,485,760 UTF-16 code units."
        )
        XCTAssertEqual(
            P08Configuration.corpusCodeUnits, 10 * (1 << 20),
            "10 MiU16 = 10 * 2^20."
        )
    }
    ...
    func testHotBlockCountIsThirty() throws {
        let manifest = makeManifest(minimumSamples: P08Configuration.hotBlocksPerCell)
        let stats = BootstrapStatistics()
        let blocks = makeBlocks(count: P08Configuration.hotBlocksPerCell)
    ...
    /// P08 is a HOT workload — its block count (30) is governed by the
    ...
    /// apply to P08's hot blocks: a hot block is a paired N/C latency, not a
    /// fresh-process launch.
    func testHotWorkloadDistinctFromColdLaunchPath() {
        XCTAssertEqual(
            P08Configuration.hotBlocksPerCell, 30,
            "P08 uses 30 HOT blocks per cell."
        )
        XCTAssertEqual(
            ColdLaunchManager.samplesPerBlock,
            P08Configuration.coldPathLaunchesPerBlock,
            "The cold path (P00) uses 50 launches per block; P08 is hot."
        )
        XCTAssertNotEqual(
            P08Configuration.hotBlocksPerCell,
            P08Configuration.coldPathLaunchesPerBlock,
    ...
    func testNoOutlierDeletionAllBlocksEnterBootstrap() throws {
        ...
        XCTAssertEqual(verdict.bootstrapDrawCount, P08Configuration.bootstrapDrawCount)
    }
    ...
    func testBootstrapDrawCountIsExactlyOneMillion() throws {
        ...
        XCTAssertEqual(
            ...
            P08Configuration.bootstrapDrawCount
        )
    ...
    /// — for P08 that means both the M0 cell and the M1 cell must pass. No
    ...
    func testRefreshCellsAreExact60And120() {
        XCTAssertEqual(
            ...
            P08Configuration.exactRefreshRates
        )
    ...
    func testBootstrapSeedMaterialIsReproducibleAndCellScoped() {
        let m0 = makeManifest(cellID: "p08-find-60hz-total-native-vs-m0")
        let m1 = makeManifest(cellID: "p08-find-60hz-total-native-vs-m1")
    ...
    // MARK: - Full P08 pipeline wiring (synthetic data — NOT a performance
    //          result)

    /// End-to-end wiring proof: the P08 pipeline — collect balanced blocks
    /// → run each verdict form → aggregate via intersection-union — is wired
    /// correctly. Uses SYNTHETIC blocks (not real find/replace timings); this
    ...
    func testP08FullPipelineWiringWithSyntheticBlocks() throws {
        let runner = BenchmarkRunner()
        let stats = BootstrapStatistics()
        ...
        let m0Manifest = makeManifest(
            cellID: "p08-find-60hz-total-native-vs-m0",
            comparator: .m0, minimumSamples: 30
        )
        let m1Manifest = makeManifest(
            cellID: "p08-find-60hz-total-native-vs-m1",
    ...
    private func makeManifest(
        cellID: String = "p08-find-60hz-total-native-vs-m0",
