// P10WorkloadTests.swift
//
// P09-T040 — Run P10: diff and multi-diff (the eleventh P-candidate).
//
// P10 is the diff and multi-diff performance benchmark: running the legacy
// and advanced diff engines on 1/10/50 MiU16 corpora with 1/10/30 percent
// changes, plus multi-diff over 1/10/100 items of 1 MiU16 (including the
// timeout and cache cells), measured as per-scan latency, then the
// 1,000,000-draw whole-balanced-block bootstrap verdict across the exact
// 60 Hz and 120 Hz frame cells for the M0 and M1 comparators, with the
// one-sided 95 percent upper bound required to be at most zero on unrounded
// binary64 values for every comparator and cell.
//
// ──────────────────────────────────────────────────────────────────────────
// FEASIBILITY ASSESSMENT (P09-T040) — Option A (structural)
// ──────────────────────────────────────────────────────────────────────────
...
//    reduced diff measurement requires the npm package + a Node.js runtime +
//    a real macOS editor session — infeasible in-session. No reduced
...
// external oracle harness); this test verifies the P10 configuration is
// correctly wired against the Phase 00 statistical + display + manifest
// infrastructure, including the legacy/advanced engine split and the
// multi-diff item-fan-out contract.
//
// Spec (verbatim, P09-T040 implementation-operation stages):
//   • Run legacy/advanced diff on 1/10/50 MiU16 with 1/10/30 percent changes
//     and multi-diff 1/10/100 items of 1 MiU16, including timeout and cache
//     cells.
...
// MARK: - P10 workload configuration (documented, verified against infra)

/// The P10 diff and multi-diff workload configuration. These are the
...
private enum P10Configuration {
    /// The two diff engines the formal run exercises — the legacy LCS-based
    /// diff (the original monaco-editor diff algorithm) and the advanced
    /// diff (the Myers / patience-hybrid diff algorithm introduced for
    /// performance). Each engine is a distinct cell axis.
    /// (spec: "Run legacy/advanced diff …")
    static let diffEngines: [String] = ["legacy", "advanced"]
    /// The three corpus sizes (in MiU16) the formal run sweeps — 1, 10, and
    /// 50. Each size is a distinct cell axis; 50 MiU16 is the heaviest single-
    /// document diff.
    /// (spec: "…on 1/10/50 MiU16 …")
    static let corpusSizesMiU16: [Int] = [1, 10, 50]
    /// 1 MiU16 = 2^20 = 1,048,576 UTF-16 code units.
    static let mebiU16CodeUnits: Int = 1 << 20
    /// The three change densities (percent of lines changed between the two
    /// diff inputs) — 1, 10, and 30 percent. Each density is a distinct cell
    /// axis.
    /// (spec: "…with 1/10/30 percent changes …")
    static let changePercents: [Double] = [1.0, 10.0, 30.0]
    /// The multi-diff item fan-outs the formal run sweeps — 1, 10, and 100
    /// documents, each of 1 MiU16, diffed in one multi-diff session. Each
    /// item count is a distinct cell axis.
    /// (spec: "…and multi-diff 1/10/100 items of 1 MiU16 …")
    static let multiDiffItemCounts: [Int] = [1, 10, 100]
    /// The two special cell axes beyond the size/percent/engine matrix —
    /// timeout (the diff aborts on a deadline-exceeded cell) and cache (the
    /// diff result is served from a warm cache vs. recomputed).
    /// (spec: "…including timeout and cache cells.")
    static let specialCells: [String] = ["timeout", "cache"]
    /// 30 hot balanced blocks per valid cell. P10 is a HOT workload — the
    ...
    /// The cold path's per-block launch count (P00 only). P10 is hot, so its
...
// MARK: - P10WorkloadTests

final class P10WorkloadTests: XCTestCase {

    // MARK: - Stage 1: legacy/advanced; 1/10/50 MiU16; 1/10/30 %; multi-diff
    //          1/10/100 items; timeout + cache cells

    /// The two diff engines — legacy (LCS-based) and advanced (Myers/patience
    /// hybrid). Each is a distinct cell axis. (spec: "Run legacy/advanced
    /// diff …")
    func testP10DiffEnginesAreLegacyAndAdvanced() {
        XCTAssertEqual(P10Configuration.diffEngines.count, 2)
        XCTAssertEqual(P10Configuration.diffEngines, ["legacy", "advanced"])
    }

    /// The three corpus sizes (in MiU16) — 1, 10, and 50. Each is a distinct
    /// cell axis; 50 MiU16 is the heaviest single-document diff.
    /// (spec: "…on 1/10/50 MiU16 …")
    func testP10CorpusSizesAreOne_Ten_FiftyMebiU16() {
        XCTAssertEqual(P10Configuration.corpusSizesMiU16.count, 3)
        XCTAssertEqual(P10Configuration.corpusSizesMiU16, [1, 10, 50])
        // Each size resolves to the exact mebi-U16 code-unit count.
        for size in P10Configuration.corpusSizesMiU16 {
            XCTAssertEqual(size * P10Configuration.mebiU16CodeUnits,
                           size * (1 << 20),
                           "\(size) MiU16 = \(size) * 2^20.")
        }
    }

    /// The three change densities — 1, 10, and 30 percent of lines changed
    /// between the two diff inputs. Each density is a distinct cell axis.
    /// (spec: "…with 1/10/30 percent changes …")
    func testP10ChangePercentsAreOne_Ten_Thirty() {
        XCTAssertEqual(P10Configuration.changePercents.count, 3)
        XCTAssertEqual(P10Configuration.changePercents, [1.0, 10.0, 30.0])
        XCTAssertEqual(P10Configuration.changePercents.first, 1.0,
                       "1% = sparse changes (near-identical inputs).")
        XCTAssertEqual(P10Configuration.changePercents.last, 30.0,
                       "30% = dense changes (heaviest diff work).")
    }

    /// The multi-diff item fan-outs — 1, 10, and 100 documents of 1 MiU16,
    /// diffed in one multi-diff session. Each item count is a distinct cell
    /// axis. (spec: "…and multi-diff 1/10/100 items of 1 MiU16 …")
    func testP10MultiDiffItemCountsAreOne_Ten_Hundred() {
        XCTAssertEqual(P10Configuration.multiDiffItemCounts.count, 3)
        XCTAssertEqual(P10Configuration.multiDiffItemCounts, [1, 10, 100])
        // Every multi-diff item is itself 1 MiU16.
        XCTAssertEqual(P10Configuration.mebiU16CodeUnits, 1 << 20,
                       "multi-diff items are each 1 MiU16 = 2^20 code units.")
    }

    /// The two special cell axes — timeout (deadline-exceeded abort) and
    /// cache (warm-cache serve vs. recompute). (spec: "…including timeout
    /// and cache cells.")
    func testP10SpecialCellsAreTimeoutAndCache() {
        XCTAssertEqual(P10Configuration.specialCells.count, 2)
        XCTAssertEqual(P10Configuration.specialCells, ["timeout", "cache"])
    }
    ...
    func testHotBlockCountIsThirty() throws {
        let manifest = makeManifest(minimumSamples: P10Configuration.hotBlocksPerCell)
        let stats = BootstrapStatistics()
        let blocks = makeBlocks(count: P10Configuration.hotBlocksPerCell)
    ...
    /// P10 is a HOT workload — its block count (30) is governed by the
    ...
    func testHotWorkloadDistinctFromColdLaunchPath() {
        XCTAssertEqual(
            P10Configuration.hotBlocksPerCell, 30,
            "P10 uses 30 HOT blocks per cell."
        )
        XCTAssertEqual(
            ColdLaunchManager.samplesPerBlock,
            P10Configuration.coldPathLaunchesPerBlock,
            "The cold path (P00) uses 50 launches per block; P10 is hot."
        )
        XCTAssertNotEqual(
            P10Configuration.hotBlocksPerCell,
            P10Configuration.coldPathLaunchesPerBlock,
    ...
    func testNoOutlierDeletionAllBlocksEnterBootstrap() throws {
        ...
        XCTAssertEqual(verdict.bootstrapDrawCount, P10Configuration.bootstrapDrawCount)
    }
    ...
    func testBootstrapDrawCountIsExactlyOneMillion() throws {
        ...
        XCTAssertEqual(
            ...
            P10Configuration.bootstrapDrawCount
        )
    ...
    /// UNROUNDED binary64 value — never on a rounded or quantized form.
    ...
    /// — for P10 that means both the M0 cell and the M1 cell must pass.
    ...
    func testRefreshCellsAreExact60And120() {
        XCTAssertEqual(
            ...
            P10Configuration.exactRefreshRates
        )
    ...
    /// changing the cellID (a different cell) changes the seed material.
    func testBootstrapSeedMaterialIsReproducibleAndCellScoped() {
        let m0 = makeManifest(cellID: "p10-diff-60hz-total-native-vs-m0")
        let m1 = makeManifest(cellID: "p10-diff-60hz-total-native-vs-m1")
    ...
    // MARK: - Full P10 pipeline wiring (synthetic data — NOT a performance
    //          result)

    /// End-to-end wiring proof: the P10 pipeline — collect balanced blocks
    /// → run each verdict form → aggregate via intersection-union — is wired
    /// correctly. Uses SYNTHETIC blocks (not real diff timings); this is a
    /// structural wiring check, NOT a performance measurement.
    func testP10FullPipelineWiringWithSyntheticBlocks() throws {
        let runner = BenchmarkRunner()
        let stats = BootstrapStatistics()
        ...
        let m0Manifest = makeManifest(
            cellID: "p10-diff-60hz-total-native-vs-m0",
            comparator: .m0, minimumSamples: 30
        )
        let m1Manifest = makeManifest(
            cellID: "p10-diff-60hz-total-native-vs-m1",
    ...
    private func makeManifest(
        cellID: String = "p10-diff-60hz-total-native-vs-m0",
